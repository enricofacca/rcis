...
def test_main(verbose=0):
    ...
    while (flags.flag >= 0):
        ...
        """
        flags, sol, grad_desc = flags.new_reverse(
            grad_desc,
            data,
            sol)
        ...
        if (flags.flag == 2):
            ...
            if (verbose):
                print('iter:', flags.iterations, 'energy=', energy, 
                      'error', abs(sol.x - data.b / data.a))
                
        if (flags.flag == 3):  
            """ Here user have to set solver controls for next update """
            grad_desc.ctrl.step = max(min(grad_desc.ctrl.step *
                                          step_expansion,
                                          max_step), min_step)
        ...
        if ( flags.flag == 4):
            ...
            grad_desc.ctrl.step = max(min(grad_desc.ctrl.step /
                                          step_contraction, max_step), min_step)
